...
export default function UserDetailPage({
  ...
  const handlePhotoConfirm = (imageFile: File, previewImage: string) => {
  ...
  const uploadImage = async () => {
    ...
    return uploadResult?.payload?.success ? uploadResult.payload.url : "";
  };

  const handleDocuments = async (selectedUserId: string) => {
  ...
  const onSubmit = async (values: EditUserFormData) => {
    ...
    try {
      ...
      await handleDocuments(selectedUser.user_id);
      setPendingCreatedDocumentDrafts([]);
      setPendingDeletedDocumentUuids([]);
        ...
        <Tabs defaultValue="details" className="w-full">
          ...
          <TabsContent value="details" className="space-y-6">
            <UserProfileHeader
              ...
              canRemoveImage={Boolean(displayedProfileImage)}
