...
module.exports = (sequelize, DataTypes) => {
  class LeaveRequest extends Model {
    ...
    static associate(models) {
      ...
      this.leave_type = LeaveRequest.belongsTo(models.leave_type, {
    ...
    isRecommended() {
      return (
        this.getDataValue("status") === LeaveRequestStatus.ENUM.RECOMMENDED
      );
    }
    ...
    approve(user) {
      if (this.isCancelled() || this.isRejected())
        throw new ForbiddenError({
          message: `Leave Request is already ${this.getDataValue("status")}`,
        });
      const changedBy = this.getDataValue("status_changed_by") || [];
      const alreadyRecommended = changedBy.some(
        (u) => u.user_id === user.user_id || u.id === user.id,
      );
      if (alreadyRecommended)
        throw new ForbiddenError({
          message: `You have already recommended this leave request.`,
        });
    ...
    recommend(user) {
      if (!this.isPending() && !this.isRecommended())
        throw new ForbiddenError({
          message: `Leave Request is already ${this.getDataValue("status")}`,
        });
      const changedBy = this.getDataValue("status_changed_by") || [];
    ...
    reject(user) {
      ...
      if (this.isCancelled() || this.isApproved())
        throw new ForbiddenError({
          message: `Leave Request is already ${this.getDataValue("status")}`,
        });
    ...
    cancel(user) {
      ...
      if (this.isApproved() || this.isRejected())
        throw new ForbiddenError({
          message: `Leave Request is ${this.getDataValue("status")}`,
        });
        ...
        validate: {
          ...
          isDateBeforeEndDate(value) {
            if (this.end_date && value > this.end_date) {
              throw new Error(
                "Start Date should be less than or equal to end_date.",
              );
            }
          },
        },
        ...
        validate: {
          ...
          isDateAfterStartDate(value) {
            if (value && value < this.start_date) {
              throw new Error(
                "End Date should be greater than or equal to start_date.",
              );
            }
          },
        },
          ...
          isRangeValidForType(value) {
            ...
            const allowedRanges = {
              ...
              [LeaveRequestType.ENUM.HALF_DAY]: [
                LeaveRange.ENUM.FIRST_HALF,
                LeaveRange.ENUM.SECOND_HALF,
              ],
            ...
            if (
              !allowedRanges[leaveType] ||
              !allowedRanges[leaveType].includes(value)
            ) {
              const validOptions = allowedRanges[leaveType]
                ? allowedRanges[leaveType].join(" or ")
                : "none";
              throw new Error(
                `Invalid Range '${value}' for Leave Type '${leaveType}'. Allowed ranges are: ${validOptions}.`,
              );
            }
          },
        },
      ...
      leave_duration: {
        ...
        validate: {
          isValid(value) {
            ...
            if (
              type === LeaveRequestType.ENUM.HALF_DAY ||
            ...
            if (value > 365)
              throw new Error(
                "Leave Duration should not be greater than 365 days.",
              );
          },
        },
        set(value) {
          ...
          if (type === LeaveRequestType.ENUM.HALF_DAY) value = 0.5;
          else if (type === LeaveRequestType.ENUM.SHORT_LEAVE) value = 0.25;
